(function () {
  ...
  if (!data) {
    document.body.innerHTML = "<p style='padding:2rem;color:#8b3a3a'>Erreur : ecoles.js introuvable.</p>";
    return;
  }
  ...
  function matchEcole(ecole) {
    ...
    if (state.origines.size > 0 && !state.origines.has(ecole.origine)) return false;
  ...
  function renderCard(ecole) {
    ...
    const restrictionBadge = ecole.restriction_creation === "interdite"
      ...
      : ecole.restriction_creation === "limitee"
        ? el("span", { class: "badge restriction-limitee", title: "Accès limité" }, "⚠ Accès limité")
        : null;
    const genreBadge = ecole.genre_restriction === "femmes"
      ? el("span", { class: "badge genre-femmes", title: "Réservée aux personnages féminins" }, "♀ Réservée aux femmes")
      : ecole.genre_restriction === "hommes"
        ? el("span", { class: "badge genre-hommes", title: "Réservée aux personnages masculins" }, "♂ Réservée aux hommes")
        : null;
    return el("li", {
      class: "ecole-card" + (ecole.enrichie ? " is-enrichie" : ""),
      tabindex: "0", role: "button",
      "aria-label": "Voir le détail de l'école " + ecole.nom,
      onclick: () => window.openItem("ecole", ecole, { resetStack: true }),
      onkeydown: (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); window.openItem("ecole", ecole, { resetStack: true }); } },
    }, [
  ...
  function updateActiveFilterCount() {
    const total = state.nations.size + state.armes.size + state.origines.size + state.restrictions.size + state.genres.size + (state.search ? 1 : 0);
    const counter = document.getElementById("filters-active-count");
    if (total > 0) { counter.textContent = total; counter.hidden = false; }
  ...
  function wireEvents() {
    ...
    document.getElementById("reset-filters").addEventListener("click", () => {
      state.search = "";
      state.nations.clear(); state.armes.clear(); state.origines.clear();
      state.restrictions.clear(); state.genres.clear();
